...
int main(int argc, char ** argv) {
	...
	//std::cout<<"argElf = " << argElf << " argKernel = "<< argKernel << std::endl;
	...
	if(flag_list) {
		//std::cout<< "flag_list = "<<flag_list<<std::endl;
	...
	//std::cout<<"kernel arch = "<<kern->arch<<std::endl;
	//std::cout<<"kernel name = "<<kern->name<<std::endl;
	...
	#else
		//sys = "cuobjdump --version | grep -E "*release*" | sed -r \"*release ([0-9]\\.[0-9])\n";
		// sys = "/usr/local/cuda-10.2/bin/cuobjdump";
		// sass = popen(sys.c_str(), "r");
		// if(!sass){
	...
	while(fgets(line, 512, sass)) {
		...
		if(strstr(line, target) && seenArch < 10) {
			...
			while(fgets(line, 512, sass)) {
				...
				if(!address) {//done with function
				...
				} else if((!strstr(address+1,"/*"))) {//process control line
					if(kern->arch >= 50 && nextaddr % (8 * 4)) {
						...
						if (kern->arch == 30){
							...
							cerr << "ERROR: SCHI value in unexpected architecture.\n";
						}
						cout << "SCHI";
						if (kern->arch < 50){
							for (int x = 0; x < 7; x++){
				...
				//address += 2;
				int currentaddr = strtoul(address+2, 0, 16);

				//std::cout<<"currentaddr: "<<std::hex<<currentaddr<<" nextaddr: "<<std::hex<<nextaddr<<std::endl; 
                ...
                special_case0 = 0;
				if(currentaddr == nextaddr && (!strstr(address+1,"/*"))){
					fgets(line2, 512, sass);
					special_case0 = 1;
					//std::cout<<"re-concat line"<<line<<std::endl;
				}
				...
				if(special_case0){
					...
					bytes = strstr(address, "/*");
				}

				if(!bytes) {
					cerr << "FATAL ERROR e2a~206.\n";
					return 1;
				}
				bytes += 2;//skip "/*"
				while((*bytes) != '*' && (*bytes) != '/') {
