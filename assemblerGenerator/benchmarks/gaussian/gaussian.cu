...
void PrintMat(FILE* file, float *ary, int nrow, int ncolumn);
void PrintAry(FILE* file, float *ary, int ary_size);
...
int main(int argc, char *argv[])
{
    ...
    FILE *fpo = fopen("verify_output","w");
    //if (verbose) {
        //printf("Matrix m is: \n");
        PrintMat(fpo, m, Size, Size);

        //printf("Matrix a is: \n");
        	PrintMat(fpo, a, Size, Size);
...
void ForwardSub()
{
	...
	fprintf(stderr, "Fan1 grid: %d\n", grid_size);
	fprintf(stderr, "Fan1 block: %d\n", block_size);
	fprintf(stderr, "Fan2 grid: %dx%d\n", gridSize2d, gridSize2d);
	fprintf(stderr, "Fan2 block: %dx%d\n", blockSize2d, blockSize2d);
 ...
 **------------------------------------------------------
 */
void PrintMat(FILE* file, float *ary, int nrow, int ncol)
{
	int i, j;
	
	for (i=0; i<nrow; i++) {
		for (j=0; j<ncol; j++) {
			fprintf(file, "%8.2f ", *(ary+Size*i+j));
		}
		fprintf(file, "\n");
	}
	fprintf(file, "\n");
