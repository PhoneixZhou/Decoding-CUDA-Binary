		...
		int total_iterations, int num_iterations, int blockCols, int blockRows, int borderCols, int borderRows) 
{
	...
	for (t = 0; t < total_iterations; t+=num_iterations) {
            ...
            src = dst;
            dst = temp;
			calculate_temp<<<dimGrid, dimBlock>>>(MIN(num_iterations, total_iterations-t), MatrixPower,MatrixTemp[src],MatrixTemp[dst], col,row,borderCols, borderRows, Cap,Rx,Ry,Rz,step,time_elapsed);
	}
	...
	{
		fprintf(stderr, "ERROR msg: %d, cudaGetErrorString: %s\n", errmsg, cudaGetErrorString(errmsg));
	}
...
void run(int argc, char** argv)
{
    ...
    printf("Start computing the transient temperature asdfaf\n");
